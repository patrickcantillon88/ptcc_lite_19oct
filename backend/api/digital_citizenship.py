#!/usr/bin/env python3
"""
Digital Citizenship API for PTCC
...
logger = get_logger("api.digital_citizenship")
router = APIRouter()
...
LOW_KEYWORDS = [
    "accidental", "first time", "forgot", "silly", "off-topic", "non-sensitive",
    "games", "social media", "cleared browser", "mildly", "vpn", "ipad",
    "learning time", "profile", "meme", "unintentional"
]

MEDIUM_KEYWORDS = [
    "cyberbully", "name-calling", "unkind", "joke meme", "sharing photos",
    "without consent", "persistent", "inappropriate search", "swear",
    "fake account", "password", "guess", "deliberate", "vpn after warning",
    "non-compliant apps", "mature topics"
]

HIGH_KEYWORDS = [
    "explicit", "pornographic", "violent", "sexting", "indecent images",
    "harassment", "threat", "impersonat", "malicious", "discriminatory",
    "racism", "homophobia", "self-harm", "radicalization", "criminal",
    "humiliate", "embarrass", "undress", "targeted", "persistent cyberbully",
    "serious", "illegal"
]


def rule_based_assessment(request: AssessmentRequest) -> Assessment:
    """Fallback rule-based assessment when AI is unavailable"""
    description = request.description.lower()
    year_group = request.year_group
    is_repeated = request.incident_history == "Repeated offense"
    
    # Check for HIGH-risk keywords first (safety override)
    high_matches = [kw for kw in HIGH_KEYWORDS if kw in description]
    if high_matches:
        classification = "HIGH"
        reason = f"Description contains high-risk keywords ({', '.join(high_matches[:2])})"
        if is_repeated:
            reason += " and this is a repeated offense"
        next_steps = [
            "Log the incident immediately in the school system",
            "Contact the Designated Safeguarding Lead (DSL) immediately",
            "Do not delay - this requires immediate attention",
            "Document all evidence and communications"
        ]
        return Assessment(classification=classification, reason=reason, next_steps=next_steps)
    
    # Check for MEDIUM-risk keywords
    medium_matches = [kw for kw in MEDIUM_KEYWORDS if kw in description]
    if medium_matches:
        base_classification = "MEDIUM"
        reason = f"Description contains concerning keywords ({', '.join(medium_matches[:2])})"
    else:
        # Check for LOW-risk keywords
        low_matches = [kw for kw in LOW_KEYWORDS if kw in description]
        if low_matches:
            base_classification = "LOW"
            reason = f"Description suggests minor incident ({', '.join(low_matches[:2])})"
        else:
            # Unknown case - default to seeking advice
            return Assessment(
                classification="MEDIUM",
                reason="Unable to clearly categorize this incident based on description",
                next_steps=[
                    "Log the incident in the school system",
                    "Seek advice from your Head of Year for appropriate next steps",
    ...
    # Escalate if repeated offense
    if is_repeated:
        if base_classification == "LOW":
            final_classification = "MEDIUM"
            reason += ". Escalated to MEDIUM due to repeated offense"
        elif base_classification == "MEDIUM":
            final_classification = "HIGH"
            reason += ". Escalated to HIGH due to repeated offense"
    
    # Apply age consideration (slight leniency for younger students)
    if year_group in ["Year 3", "Year 4"] and not is_repeated:
        reason += f". {year_group} student - foundational digital citizenship skills still developing"
    
    # Generate appropriate next steps
    if final_classification == "LOW":
        next_steps = [
            "Log the incident in the school system",
            "Speak with the student about appropriate digital citizenship",
            "Provide guidance on correct procedures",
            "Monitor for any repeat incidents"
        ]
    elif final_classification == "MEDIUM":
        next_steps = [
            "Log the incident in the school system",
            "Inform your Head of Year about this incident",
            "Arrange a conversation with the student and possibly parents/guardians",
            "Implement appropriate consequences as per school policy",
            "Monitor the student's digital activity more closely"
        ]
    else:  # HIGH
        next_steps = [
            "Log the incident immediately in the school system",
            ...
            "Do not delay - this requires immediate escalation"
        ]
...
async def assess_incident(request: AssessmentRequest) -> Assessment:
    ...
    try:
        ...
        # Try to get Gemini client - if it fails, use fallback
        try:
            genai = get_gemini_client()
        except Exception as ai_error:
            logger.warning(f"AI service unavailable: {ai_error}. Using rule-based fallback.")
            fallback_assessment = rule_based_assessment(request)
            logger.info(f"Rule-based assessment complete: {fallback_assessment.classification}")
...
"""
        ...
        logger.warning(f"AI assessment failed: {e}. Falling back to rule-based assessment.")
        ...
        try:
            ...
            logger.error(f"Both AI and rule-based assessment failed: {fallback_error}", exc_info=True)
            raise HTTPException(
                status_code=500,
                detail=f"Assessment system unavailable: {str(fallback_error)}"
            )


@router.get("/health")
async def health_check():
    """Check if Digital Citizenship API and Gemini service are available"""
    try:
        api_key = os.getenv("GEMINI_API_KEY")
        if not api_key:
            return {
        ...
        return {
            ...
            "message": "Digital Citizenship API is operational"
        }
    except Exception as e:
        return {
            "status": "unhealthy",
